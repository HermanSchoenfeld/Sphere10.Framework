...
public abstract class DigestTest {
	...
	protected virtual void PerformTest() {
		...
		Assert.That(resBuf, Is.EqualTo(lastDigest), string.Format("fail clone vector test, expected {0} but got {1}",
		...
		Assert.That(resBuf, Is.EqualTo(lastDigest), string.Format("fail second clone vector test, expected {0} but got {1}",
				_results[_results.Length - 1],
				Hex.ToHexString(resBuf)));
		...
		Assert.That(resBuf, Is.EqualTo(lastDigest), string.Format("fail memo vector test, expected {0} but got {1}",
				_results[_results.Length - 1],
				Hex.ToHexString(resBuf)));
		...
		Assert.That(resBuf, Is.EqualTo(lastDigest), string.Format("fail memo reset vector test, expected {0} but got {1}",
				_results[_results.Length - 1],
				Hex.ToHexString(resBuf)));
		...
		Assert.That(resBuf, Is.EqualTo(lastDigest), string.Format("fail memo copy vector test, expected {0} but got {1}",
				_results[_results.Length - 1],
				Hex.ToHexString(resBuf)));
	}
	...
	private void vectorTest(
		...
		Assert.That(expected, Is.EqualTo(resBuf), string.Format("Vector {0} failed, got {1}", count, Hex.ToHexString(resBuf)));
	}
