...
```bash
dotnet add package Sphere10.Framework.Windows.Forms.MySQL
```
...
Build with the .NET 10 SDK. Create and use controls on an STA UI thread. A reachable MySQL server and suitable credentials are required only when testing or opening a database connection; constructing and editing a control does not open one.

## Package architecture

| Component | Responsibility |
|---|---|
| [`MySQLConnectionBar`](MySQLConnectionBar.cs) | Compact editor derived from `ConnectionBarBase`; includes its inherited artificial-key options |
| [`MySQLConnectionPanel`](MySQLConnectionPanel.cs) | Larger table-layout editor derived from `ConnectionPanelBase` |
| [`ModuleConfiguration`](ModuleConfiguration.cs) | Registers both controls as named transient services under `nameof(DBMSType.MySQL)` |
...
| `Sphere10.Framework.Data.MySQL` | `MySQLDAC`, SQL-building/database support, and the `MySqlConnector` driver dependency |

The UI classes are in `Sphere10.Framework.Windows.Forms.MySQL`. The returned `MySQLDAC` is in `Sphere10.Framework.Data`. Install the data-provider package by itself for applications that do not need Windows Forms.
...
| Member | Meaning |
|---|---|
| `Server` | MySQL server name; maps to `MySqlConnectionStringBuilder.Server` |
| `Database` / `DatabaseName` | Database field; `DatabaseName` returns its current text |
| `Username`, `Password` | Map to the provider's `UserID` and `Password` |
| `Port` | Text field; blank omits an explicit port, using the provider default of 3306 |
| `ConnectionString` | Builds or loads the five supported fields using `MySqlConnectionStringBuilder` |
| `GetDAC()` | Returns a new `MySQLDAC` through `IDAC`, using the control's current connection string |
...
Assigning `ConnectionString` populates the displayed fields. A provider-default port (`3306`) is shown as blank; a nondefault port is shown explicitly.
...
using Sphere10.Framework.Windows.Forms.MySQL;
using WinFormsApplication = System.Windows.Forms.Application;

namespace ReadmeExamples.MySQLDirect;
...
internal static class Program {
	...
	private static void Main() {
		...
		using var window = new Form { Text = "MySQL connection", ClientSize = new Size(640, 260) };
		var editor = new MySQLConnectionPanel {
			Dock = DockStyle.Fill,
			Server = "localhost",
			Database = "app_data",
			Username = "app_user",
...
To use a bar instead, host `MySQLConnectionBar` in your form and call the same `ConnectionString`, `GetDAC()`, and `TestConnection()` members. The bar additionally inherits `ArtificialKeysFile` and `SelectArtificialKeysFile()`; a configured artificial-key file is loaded when `GetDAC()` is called. The panel has no corresponding artificial-key file property.
...
using ProviderModule = Sphere10.Framework.Windows.Forms.MySQL.ModuleConfiguration;
using WinFormsModule = Sphere10.Framework.Windows.Forms.ModuleConfiguration;
using WinFormsApplication = System.Windows.Forms.Application;

namespace ReadmeExamples.MySQLSelector;
...
internal static class Program {
	...
	private static void Main() {
		...
		var selector = new DatabaseConnectionPanel {
			Dock = DockStyle.Fill,
			SelectedDBMSType = DBMSType.MySQL
		};
...
- The underlying `MySQLDAC.BulkInsert` currently throws `NotImplementedException`. See the data-provider documentation for supported SQL, schema, and database-management operations; adding these UI controls does not add missing provider functionality.
...
```bash
dotnet build src/Sphere10.Framework.Windows.Forms.MySQL/Sphere10.Framework.Windows.Forms.MySQL.csproj -c Release
dotnet pack src/Sphere10.Framework.Windows.Forms.MySQL/Sphere10.Framework.Windows.Forms.MySQL.csproj -c Release --no-build
```
...
- [MySQL data provider](../Sphere10.Framework.Data.MySQL/README.md)
