...
public class SIDSearcher {
	...
	public void SearchEntireRegistry() {
		if (Tools.Windows.Registry.KeyExists(Hostname, "HKEY_CLASSES_ROOT")) {
			SearchRegistry("HKEY_CLASSES_ROOT", true);
		}
		if (Tools.Windows.Registry.KeyExists(Hostname, "HKEY_CURRENT_USER")) {
			SearchRegistry("HKEY_CURRENT_USER", true);
		}

		if (Tools.Windows.Registry.KeyExists(Hostname, "HKEY_LOCAL_MACHINE")) {
			SearchRegistry("HKEY_LOCAL_MACHINE", true);
		}

		if (Tools.Windows.Registry.KeyExists(Hostname, "HKEY_USERS")) {
			SearchRegistry("HKEY_USERS", true);
		}

		if (Tools.Windows.Registry.KeyExists(Hostname, "HKEY_CURRENT_CONFIG")) {
			SearchRegistry("HKEY_CURRENT_CONFIG", true);
		}

		if (Tools.Windows.Registry.KeyExists(Hostname, "HKEY_DYN_DATA")) {
	...
	public void SearchRegistry(string rootKey, bool recursive) {
		try {
			...
			RegistryKey key = Tools.Windows.Registry.OpenKey(Hostname, rootKey);
			try {
			...
			if (recursive) {
				foreach (string subKey in Tools.Windows.Registry.GetSubKeys(Hostname, rootKey)) {
					if (Tools.Windows.Registry.KeyExists(Hostname, subKey)) {
