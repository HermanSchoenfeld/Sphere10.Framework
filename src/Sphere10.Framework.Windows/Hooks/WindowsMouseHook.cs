			...
			try {
				...
				if (nCode >= 0) {
					...
					if ((mouseMessages & WinAPI.USER32.WM.MOUSEWHEEL) == WinAPI.USER32.WM.MOUSEWHEEL) {
						wheelDelta = Tools.Windows.Win32.GET_WHEEL_DELTA_WPARAM(mouseHookStruct.mouseData) * WinAPI.USER32.WHEEL_DELTA;
					} else {
						...
						if ((mouseMessages & WinAPI.USER32.WM.XBUTTONDOWN) == WinAPI.USER32.WM.XBUTTONDOWN) {
							buttonClicked = (Tools.Windows.Win32.HIWORD(mouseHookStruct.mouseData) & (ushort)WinAPI.USER32.MouseEventDataXButtons.XBUTTON1) == (ushort)WinAPI.USER32.MouseEventDataXButtons.XBUTTON1
						...
						if ((mouseMessages & WinAPI.USER32.WM.XBUTTONUP) == WinAPI.USER32.WM.XBUTTONUP) {
							buttonClicked = (Tools.Windows.Win32.HIWORD(mouseHookStruct.mouseData) & (ushort)WinAPI.USER32.MouseEventDataXButtons.XBUTTON2) == (ushort)WinAPI.USER32.MouseEventDataXButtons.XBUTTON2
								? MouseButton.XButton1
								: MouseButton.XButton2;
							buttonState = MouseButtonState.Up;
							clickType = (mouseMessages & WinAPI.USER32.WM.XBUTTONDBLCLK) == WinAPI.USER32.WM.XBUTTONDBLCLK ? MouseClickType.Double : MouseClickType.Single;
